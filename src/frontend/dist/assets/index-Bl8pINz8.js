...
const INVINCIBLE_MS = 2e3;
const MAX_SPEED = BASE_SPEED + 18 * 20;
...
function drawSun(ctx, glowPulse) {
  const sx = CANVAS_W - 100;
  const sy = 60;
  const pulse = 1 + 0.06 * Math.sin(glowPulse * 3e-3);
...
function drawMountains(ctx, mountains, bgScrollX) {
  ctx.save();
...
function drawBush(ctx, x, y, w, far) {
  const h = w * 0.5;
...
function drawGoomba(ctx, x, y, w, h, frame) {
...
function drawSonic(ctx, sonic, blinkOn) {
  if (!blinkOn) return;
  const x = sonic.x;
  const y = sonic.y;
...
function drawGround(ctx, segments) {
...
function pixelFont(ctx, text, x, y, size, color, align = "left") {
...
function App() {
  ...
  const gRef = reactExports.useRef(createGame());
  ...
  reactExports.useEffect(() => {
    ...
    function spawnObstacle(g2) {
      const r = Math.random();
      const id = ++g2.idCounter;
      if (r < 0.35) {
        g2.goombas.push({
          id,
          x: CANVAS_W + 20,
        ...
        g2.gaps.push({ id, x: CANVAS_W + 10, w: gapW });
      }
    }
    function spawnMushroom(g2) {
      if (g2.sonic.lives >= 3) return;
      const id = ++g2.idCounter;
      g2.mushrooms.push({
        id,
        x: CANVAS_W + 10,
    ...
    function hitSonic(g2) {
      if (g2.sonic.invincible > 0) return;
      g2.sonic.lives -= 1;
      if (g2.sonic.lives <= 0) {
        g2.sonic.dead = true;
        g2.sonic.deadTimer = 1e3;
        g2.sonic.vy = JUMP_VY * 0.8;
      } else {
        g2.sonic.invincible = INVINCIBLE_MS;
      }
    }
    function update(dt, g2) {
      ...
      const spd = g2.speed;
      g2.score += dt * 0.06;
      g2.speed = BASE_SPEED + Math.floor(g2.score / 500) * 18;
      g2.glowPulse += dt;
      const milestone = Math.floor(g2.score / 500) * 500;
      if (milestone > 0 && milestone > g2.lastScorePop) {
        g2.lastScorePop = milestone;
        g2.scorePops.push({ value: 500, x: CANVAS_W - 80, y: 80, timer: 900 });
      }
      g2.scorePops = g2.scorePops.filter((p) => {
        p.timer -= dt;
        p.y -= dt * 0.04;
        return p.timer > 0;
      });
      g2.bgScrollX += spd * dt * 1e-3 * 0.5;
      for (const c of g2.clouds) {
        c.x -= c.speed * dt * 1e-3;
        if (c.x + c.w < 0) c.x = CANVAS_W + c.w;
      }
      for (const b of g2.bushes) {
        const bspd = b.far ? spd * 0.35 : spd;
        b.x -= bspd * dt * 1e-3;
        if (b.x + b.w < 0) b.x = CANVAS_W + b.w + 10;
      }
      for (const seg of g2.ground) {
        seg.x -= spd * dt * 1e-3;
      }
      const lastSeg = g2.ground[g2.ground.length - 1];
      if (lastSeg && lastSeg.x + lastSeg.w < CANVAS_W + 400) {
        g2.ground.push({ x: lastSeg.x + lastSeg.w, w: 400 });
      }
      g2.ground = g2.ground.filter((s) => s.x + s.w > 0);
      if (!m.dead) {
        m.vy += GRAVITY * dt * 1e-3;
        m.y += m.vy * dt * 1e-3;
        ...
        if (m.deadTimer <= 0 || m.y > CANVAS_H + 60) {
          ...
          if (g2.score > g2.highScore) {
            ...
            localStorage.setItem("sonic-hs", String(Math.floor(g2.score)));
          }
        }
      }
      g2.spawnTimer -= dt;
      if (g2.spawnTimer <= 0) {
        spawnObstacle(g2);
        g2.spawnTimer = 1e3 + Math.random() * 1200 - Math.floor(g2.score / 1e3) * 80;
        g2.spawnTimer = Math.max(g2.spawnTimer, 500);
      }
      g2.mushroomTimer -= dt;
      if (g2.mushroomTimer <= 0) {
        spawnMushroom(g2);
        g2.mushroomTimer = 8e3 + Math.random() * 6e3;
      }
      g2.goombas = g2.goombas.filter((gb) => {
        gb.x -= spd * dt * 1e-3;
        gb.walkTimer += dt;
        if (gb.walkTimer > 200) {
        ...
        if (!m.dead && aabb(m.x + 4, m.y + 4, m.w - 8, m.h - 4, gb.x, gb.y, gb.w, gb.h)) {
          hitSonic(g2);
          return false;
        }
        return gb.x + gb.w > -10;
      });
      g2.pipes = g2.pipes.filter((p) => {
        p.x -= spd * dt * 1e-3;
        if (!m.dead && aabb(m.x + 4, m.y + 4, m.w - 8, m.h - 4, p.x, p.y, p.w, p.h)) {
          hitSonic(g2);
          p.x = -200;
        }
        return p.x + p.w > -10;
      });
      g2.gaps = g2.gaps.filter((gp) => {
        gp.x -= spd * dt * 1e-3;
        return gp.x + gp.w > -10;
      });
      g2.mushrooms = g2.mushrooms.filter((mu) => {
        mu.x -= spd * dt * 1e-3;
        if (!mu.collected && !m.dead && aabb(m.x, m.y, m.w, m.h, mu.x, mu.y, mu.w, mu.h)) {
          mu.collected = true;
    ...
    function drawHUD(ctx2, g2) {
      ctx2.save();
      ctx2.shadowColor = "rgba(0,150,255,0.4)";
      ctx2.shadowBlur = 12;
      const scoreGrad = ctx2.createLinearGradient(
        CANVAS_W - 160,
        10,
        CANVAS_W - 10,
        62
      ...
      ctx2.fillStyle = scoreGrad;
      ctx2.beginPath();
      ctx2.roundRect(CANVAS_W - 160, 10, 150, 52, 8);
      ctx2.fill();
      ctx2.strokeStyle = "rgba(100,180,255,0.5)";
      ctx2.lineWidth = 1.5;
      ctx2.beginPath();
      ctx2.roundRect(CANVAS_W - 160, 10, 150, 52, 8);
      ctx2.stroke();
      ctx2.restore();
      pixelFont(ctx2, "SCORE", CANVAS_W - 150, 30, 7, "#FFD700");
      pixelFont(
        ctx2,
        String(Math.floor(g2.score)).padStart(6, "0"),
        CANVAS_W - 150,
        52,
        10,
        "#FFFFFF"
      ...
      ctx2.shadowColor = "rgba(0,150,255,0.4)";
      ctx2.shadowBlur = 12;
      ctx2.fillStyle = "rgba(10,20,50,0.78)";
      ctx2.beginPath();
      ctx2.roundRect(CANVAS_W / 2 - 80, 10, 160, 52, 8);
      ctx2.fill();
      ctx2.strokeStyle = "rgba(100,180,255,0.5)";
      ctx2.lineWidth = 1.5;
      ctx2.beginPath();
      ...
      ctx2.stroke();
      ctx2.restore();
      pixelFont(ctx2, "HI-SCORE", CANVAS_W / 2 - 70, 30, 7, "#FFD700");
      pixelFont(
        ctx2,
        String(Math.floor(g2.highScore)).padStart(6, "0"),
        CANVAS_W / 2 - 70,
        52,
        10,
        "#FFFFFF"
      ...
      ctx2.save();
      ctx2.shadowColor = "rgba(255,50,100,0.4)";
      ctx2.shadowBlur = 10;
      ctx2.fillStyle = "rgba(10,20,50,0.78)";
      ctx2.beginPath();
      ctx2.roundRect(10, 10, 110, 44, 8);
      ctx2.fill();
      ctx2.strokeStyle = "rgba(255,100,150,0.5)";
      ctx2.lineWidth = 1.5;
      ctx2.beginPath();
      ctx2.roundRect(10, 10, 110, 44, 8);
      ctx2.stroke();
      ctx2.restore();
      for (let i = 0; i < 3; i++)
        drawHeart(ctx2, 18 + i * 34, 18, 24, i < g2.sonic.lives);
      const barW = 200;
      const barX = CANVAS_W / 2 - barW / 2;
      const barY = CANVAS_H - 16;
      const speedRatio = Math.min(
        (g2.speed - BASE_SPEED) / (MAX_SPEED - BASE_SPEED),
        1
      );
      ctx2.fillStyle = "rgba(0,0,0,0.45)";
      ctx2.beginPath();
      ctx2.roundRect(barX, barY, barW, 8, 4);
      ctx2.fill();
      const sbarGrad = ctx2.createLinearGradient(barX, 0, barX + barW, 0);
      sbarGrad.addColorStop(0, "#00cfff");
      sbarGrad.addColorStop(0.6, "#0057B8");
      sbarGrad.addColorStop(1, "#ff3b3b");
      ctx2.fillStyle = sbarGrad;
      ctx2.beginPath();
      ctx2.roundRect(barX, barY, barW * speedRatio + 4, 8, 4);
      ctx2.fill();
      ...
      ctx2.roundRect(barX, barY, barW, 8, 4);
      ctx2.stroke();
      for (const pop2 of g2.scorePops) {
        const alpha = Math.min(1, pop2.timer / 300);
        ctx2.save();
        ctx2.globalAlpha = alpha;
        ctx2.shadowColor = "#FFD700";
        ctx2.shadowBlur = 16;
        ctx2.font = "bold 14px 'Press Start 2P', monospace";
        ctx2.textAlign = "center";
        ctx2.fillStyle = "#FFD700";
        ctx2.fillText(`+${pop2.value}`, pop2.x, pop2.y);
        ctx2.restore();
      }
    }
    function drawStartScreen(ctx2, g2) {
      const overlayGrad = ctx2.createLinearGradient(0, 0, 0, CANVAS_H);
      ...
      ctx2.fillStyle = overlayGrad;
      ctx2.fillRect(0, 0, CANVAS_W, CANVAS_H);
      ctx2.save();
      ctx2.shadowColor = "#0057B8";
      ctx2.shadowBlur = 30;
      const panelGrad = ctx2.createLinearGradient(
        CANVAS_W / 2 - 200,
        90,
        CANVAS_W / 2 + 200,
        290
      ...
      ctx2.fillStyle = panelGrad;
      ctx2.beginPath();
      ctx2.roundRect(CANVAS_W / 2 - 210, 90, 420, 210, 16);
      ctx2.fill();
      ctx2.restore();
      ctx2.strokeStyle = "rgba(0,140,255,0.6)";
      ctx2.lineWidth = 2;
      ctx2.beginPath();
      ctx2.roundRect(CANVAS_W / 2 - 210, 90, 420, 210, 16);
      ctx2.stroke();
      const pulse = 0.7 + 0.3 * Math.sin(g2.glowPulse * 4e-3);
      ctx2.save();
      ctx2.shadowColor = "#0057B8";
      ctx2.shadowBlur = 30 * pulse;
      pixelFont(ctx2, "SONIC", CANVAS_W / 2, 152, 26, "#2979ff", "center");
      ctx2.restore();
      ctx2.save();
      ctx2.shadowColor = "#FFFFFF";
      ctx2.shadowBlur = 14 * pulse;
      pixelFont(ctx2, "RUNNER", CANVAS_W / 2, 188, 26, "#FFFFFF", "center");
      ctx2.restore();
    ...
    function drawGameOver(ctx2, g2) {
      ...
      ctx2.fillStyle = panelGrad;
      ctx2.beginPath();
      ctx2.roundRect(CANVAS_W / 2 - 210, 70, 420, 270, 16);
      ctx2.fill();
      ctx2.restore();
      ctx2.strokeStyle = "rgba(229,42,30,0.7)";
      ctx2.lineWidth = 2;
      ctx2.beginPath();
      ctx2.roundRect(CANVAS_W / 2 - 210, 70, 420, 270, 16);
      ctx2.stroke();
      ...
      ctx2.save();
      ctx2.shadowColor = "#E52A1E";
      ctx2.shadowBlur = 28 * pulse;
      pixelFont(ctx2, "GAME OVER", CANVAS_W / 2, 132, 18, "#E52A1E", "center");
      ctx2.restore();
      pixelFont(
        ctx2,
        `SCORE: ${Math.floor(g2.score).toString().padStart(6, "0")}`,
        CANVAS_W / 2,
        172,
        9,
        "#FFFFFF",
      ...
      pixelFont(
        ctx2,
        `BEST:  ${Math.floor(g2.highScore).toString().padStart(6, "0")}`,
        CANVAS_W / 2,
        198,
        9,
        "#FFD700",
      ...
      ctx2.shadowColor = "#E52A1E";
      ctx2.shadowBlur = 16;
      const btnGrad = ctx2.createLinearGradient(
        CANVAS_W / 2 - 100,
        228,
        CANVAS_W / 2 + 100,
        276
      ...
      ctx2.fillStyle = btnGrad;
      ctx2.beginPath();
      ctx2.roundRect(CANVAS_W / 2 - 100, 228, 200, 48, 8);
      ctx2.fill();
      ctx2.restore();
      ctx2.strokeStyle = "rgba(255,120,100,0.6)";
      ctx2.lineWidth = 1.5;
      ctx2.beginPath();
      ctx2.roundRect(CANVAS_W / 2 - 100, 228, 200, 48, 8);
      ctx2.stroke();
      pixelFont(ctx2, "PLAY AGAIN", CANVAS_W / 2, 260, 9, "#FFFFFF", "center");
    }
    function draw(g2) {
      if (!ctx) return;
      drawSkyGradient(ctx);
      drawSun(ctx, g2.glowPulse);
      drawMountains(ctx, g2.mountains, g2.bgScrollX);
      for (const c of g2.clouds) drawCloud(ctx, c.x, c.y, c.w);
      for (const b of g2.bushes) {
        if (b.far) drawBush(ctx, b.x, b.y, b.w, true);
      }
      drawGround(ctx, g2.ground);
      for (const gap of g2.gaps) {
      ...
      for (const p of g2.pipes) drawPipe(ctx, p.x, p.y, p.w, p.h);
      for (const mu of g2.mushrooms) drawMushroom(ctx, mu.x, mu.y, mu.w, mu.h);
      for (const gb of g2.goombas)
        drawGoomba(ctx, gb.x, gb.y, gb.w, gb.h, gb.walkFrame);
      const blinkOn = g2.sonic.invincible <= 0 || Math.floor(g2.sonic.invincible / 120) % 2 === 0;
      drawSonic(ctx, g2.sonic, blinkOn);
      drawHUD(ctx, g2);
      if (g2.state === "start") drawStartScreen(ctx, g2);
      if (g2.state === "gameover") drawGameOver(ctx, g2);
    }
    function gameLoop(timestamp) {
      const g2 = gRef.current;
      if (g2.paused || g2.state === "start" || g2.state === "gameover") {
        g2.glowPulse += 16;
        draw(g2);
        g2.frameId = requestAnimationFrame(gameLoop);
        return;
      }
      const dt = Math.min(timestamp - g2.lastTime, 50);
      g2.lastTime = timestamp;
      update(dt, g2);
      draw(g2);
      g2.frameId = requestAnimationFrame(gameLoop);
    }
  ...
  return /* @__PURE__ */ jsxRuntimeExports.jsxs(
    "div",
    {
      style: {
        minHeight: "100vh",
        background: "linear-gradient(160deg, #050818 0%, #0b1a3a 50%, #0d0d20 100%)",
        display: "flex",
            ...
            children: [
              /* @__PURE__ */ jsxRuntimeExports.jsx(
                "span",
                {
                  ...
                  children: "⚡ SONIC RUNNER"
                }
              ),
              /* @__PURE__ */ jsxRuntimeExports.jsx("span", { style: { color: "#6a8aaa", fontSize: 9 }, children: "SPACE / TAP TO JUMP" })
            ]
          }
        ),
        /* @__PURE__ */ jsxRuntimeExports.jsx(
          "div",
          {
            style: {
              ...
              borderRadius: 12,
              overflow: "hidden",
              boxShadow: "0 0 60px rgba(0,87,184,0.6), 0 0 120px rgba(0,30,90,0.8), 0 20px 60px rgba(0,0,0,0.9)",
